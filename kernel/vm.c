 ...
 */
pagetable_t kernel_pagetable;  //一个根页表的实际物理地址 STAP
...
walk(pagetable_t pagetable, uint64 va, int alloc)
{
  ...
  for(int level = 2; level > 0; level--) {
    ...
    if(*pte & PTE_V) {
      ...
      *pte = PA2PTE(pagetable) | PTE_V;  //没有下一级的4096的页面我新建一个，然后把
    } 
  }
  return &pagetable[PX(0, va)];  //第三级的PTE
}
...
mappages(pagetable_t pagetable, uint64 va, uint64 size, uint64 pa, int perm)
{
  ...
  a = PGROUNDDOWN(va);   //清楚最低的12wei
  last = PGROUNDDOWN(va + size - 1);
  for(;;){
    if((pte = walk(pagetable, a, 1)) == 0)  //返回的是最高级对应的地方的值
      return -1;
    if(*pte & PTE_V)
      panic("remap");
...
void
uk_freepagetable(pagetable_t ukpagetable)
{
   ...
   for(int i = 0; i < 512; i++){
    ...
    if((pte & (PTE_R|PTE_W|PTE_X)) == 0){
      // this PTE points to a lower-level page table.
      uint64 child = PTE2PA(pte);
      uk_freepagetable((pagetable_t)child);
    } 
    }
  }
